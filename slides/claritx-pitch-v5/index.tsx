...
const RiskCard = ({
  ...
  >
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
      <span style={{ color: C.red, fontFamily: 'var(--osd-font-display)', fontWeight: 700, fontSize: 22 }}>▲</span>
      <div style={{ fontFamily: 'var(--osd-font-display)', fontSize: 22, fontWeight: 600, color: C.text, letterSpacing: '-0.01em' }}>
        {title}
      </div>
    </div>
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, paddingTop: 8, borderTop: `1px solid ${C.ruleSoft}` }}>
      <span style={{ color: C.green, fontFamily: 'var(--osd-font-display)', fontWeight: 700, fontSize: 22, marginTop: 4 }}>▼</span>
      <div style={{ fontSize: 16, color: C.muted, lineHeight: 1.5 }}>{body}</div>
    </div>
  </div>
);
...
const Risks: Page = () => (
  <div style={fill}>
    <div style={{ position: 'absolute', inset: 0, padding: '88px 96px 110px' }}>
      <Eyebrow section="23 / 24">Risks · and how we hedge them</Eyebrow>
      <Heading size={52}>
      ...
      >
        <RiskCard
          title="Regulatory"
          delay={260}
          body="We publish research, not advice. No PFOF, no advisory license required (US publisher exemption). Legal counsel engaged · structured compliance review on the build agenda."
        />
        <RiskCard
          title="Model drift"
          delay={330}
          body="Every AI read cites its source. Evaluation harness in place pre-seed. Multi-model architecture (Claude + GPT + Gemini), no single-vendor lock-in."
        />
        <RiskCard
          title="Affiliate concentration"
          delay={400}
          body="eToro live today. Robinhood, IBKR, Webull onboarding in 12 months. Match analysis's asset class to best-converting broker, no single-partner dependency."
        />
      </div>
    </div>
    <Watermark />
...
const FactCard = ({
  ...
  <Card delay={delay} accent={C.green} style={{ padding: '22px 26px', display: 'flex', flexDirection: 'column', gap: 10 }}>
    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.22em', color: C.green, textTransform: 'uppercase' }}>
      {num}
    </div>
    <div style={{ fontFamily: 'var(--osd-font-display)', fontSize: 20, fontWeight: 600, color: C.text, letterSpacing: '-0.01em' }}>
      {title}
    </div>
    <div style={{ fontSize: 16, color: C.muted, lineHeight: 1.45 }}>{body}</div>
  </Card>
);
...
const Today: Page = () => (
  <div style={fill}>
    <div style={{ position: 'absolute', inset: 0, padding: '88px 96px 110px' }}>
      <Eyebrow section="23 / 24">What's already true today</Eyebrow>
      <Heading size={56}>
        This isn't a deck. <GreenAccent>It's a shipped business.</GreenAccent>
      </Heading>
      <Lede>
        Four facts that de-risk the "is this real?" question, before any conversation about traction.
      </Lede>
      ...
      >
        <FactCard
          ...
          body="claritx.ai shipped Oct 2025. 9-dimension AI pipeline running in production on AWS."
          delay={240}
        />
        <FactCard
          num="02"
          title="eToro affiliate live"
          body="Affiliate routing already running across the site. First revenue line is operational."
          delay={300}
        />
        <FactCard
          num="03"
          title="Brand & content shipped"
          body="Pricing page, blog engine, daily ranking infrastructure. Habit-loop substrate already built."
          delay={360}
        />
        <FactCard
          num="04"
          title="Clean foundation"
          body="Clean cap table · no debt · 3 full-time founders on equity since 2025 · AWS + Google for Startups credits cover infra for 18+ months."
          delay={420}
        />
      </div>
    </div>
